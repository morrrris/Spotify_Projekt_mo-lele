...
artist_per_month <- function(artist_name, data = all_data){
  ...
  p <- ggplot(ergebnisse, aes(x=as.factor(id), y=Zeit, fill = Jahr)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
